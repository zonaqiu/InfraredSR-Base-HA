...
class Trainer():
    ...
    def train(self, current_epoch=0, is_init=False):
        ...
        self.model.train() # 在训练中必须写
        if (not is_init):
            self.scheduler.step() # 只有用了optimizer.step()模型才会更新，scheduler.step()是对 lr 进行调整
        ...
        for i_batch, sample_batched in enumerate(self.dataloader['train']):
            ...
            sample_batched = self.prepare(sample_batched) # 移动到 cuda
            ...
            if (not is_init):
                ...
                writer.add_scalar('rec_loss',rec_loss.item(),global_step = i_batch+current_epoch*i_batch)
                writer.add_scalar('per_loss',per_loss.item(),global_step = i_batch+current_epoch*i_batch)
                # writer.add_scalar('adv_loss',adv_loss.item(),global_step = i_batch+current_epoch*i_batch)
                writer.add_scalar('toal_loss',loss.item(),global_step = i_batch+current_epoch*i_batch)
            ...
            with torch.no_grad():
                ...
                for i_batch, sample_batched in enumerate(self.dataloader['test']['1']):
                    ...
                    if (self.args.eval_save_results):
                        ...
                        imsave(os.path.join(self.args.save_dir, 'save_results', os.path.basename(self.args.lr_path)), sr_save)
                ...
                writer.add_scalar('PSNR',psnr_ave,global_step = current_epoch)
                writer.add_scalar('SSIM',ssim_ave,global_step = current_epoch)
    ...
    def test(self):
        ...
        for LR_filename in os.listdir(self.args.lr_path):
            # print(LR_filename[:-6]+'1.jpg')
            # exit(0)
            # print(os.path.join(self.args.save_dir, 'save_results', LR_filename))
            # exit(0)
            # LR = imread(self.args.lr_path+LR_filename)
            LR = cv.imread(self.args.lr_path+LR_filename,0)
            h1, w1 = LR.shape[:2]
            h1, w1 = h1 // 4 * 4, w1 // 4 * 4

            LR = np.array(Image.fromarray(LR).resize((w1 // 4, h1 // 4), Image.BICUBIC))
            ...
            ### Ref and Ref_sr
            Ref_filename = LR_filename[:-6]+'1.jpg'
            # Ref = imread(self.args.ref_path+Ref_filename)
            Ref = cv.imread(self.args.ref_path+Ref_filename,0)
            # exit(0)
            h2, w2 = Ref.shape[:2]
            h2, w2 = h2//4*4, w2//4*4
            Ref = Ref[:h2, :w2]
            ...
            with torch.no_grad():
                ...
                sr_save = (sr+1.) * 127.5
                sr_save = np.transpose(sr_save.squeeze().round().cpu().numpy(), (1, 2, 0)).astype(np.uint8)
                save_path = os.path.join(self.args.save_dir, 'save_results', LR_filename)
