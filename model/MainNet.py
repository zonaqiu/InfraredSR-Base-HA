...
class ResidualGroup(nn.Module):
    def __init__(self, n_feat, reduction, act, n_resblocks):
        ...
        # modules_body = []
        # self.conv = conv(n_feat // 2, n_feat, kernel_size)
        self.conv = conv3x3(n_feat//2,n_feat)
        modules_body = [
...
class MainNet(nn.Module):
    def __init__(self,args, num_res_blocks, n_feats, res_scale):
        ...
        modules_head = [conv3x3(n_feats,n_feats//2)]
        ...
        # define tail module
        modules_tail = [
            Upsampler(scale, n_feats//2, act=False),
            conv3x3(n_feats//2,args.n_colors)]
        ...
        self.last_conv = nn.Conv2d(self.nf * n_resgroups , n_feats, 3, 1, 1)
        self.mid_conv = conv3x3(n_feats//2,n_feats)
        self.last = nn.Conv2d(n_feats * 2, n_feats//2, 3, 1, 1)
    ...
    def forward(self, x, S=None, T_lv3=None):
        ...
        x = self.head(x11_res) # 3 * 3 卷积  32 channels
        res_for = x
        # pdb.set_trace()
        for name, midlayer in self.body._modules.items():
            res = midlayer(res_for)
            res_up = res[:, 0:self.nf, :, :]  # 32 channel
            res_for = res[:, self.nf:, :, :]  # 32 channel
            # print(name)
            if name == '0':
                res1 = res_up.unsqueeze(1) # 在第一维增加一个维度
            else:
                res1 = torch.cat([res_up.unsqueeze(1), res1], 1)
        # res = self.body(x)
        out1 = self.mid_conv(res_for) # 从 RG出来的，继续向前走的部分
